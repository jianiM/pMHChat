...
if __name__ == "__main__":
    ...
    for fold in range(5):
        ...
        for epoch in range(1, epochs + 1):
            ...
            _,_,_,_,_,_,val_auc, val_aupr,_, _, _, _, _ = eval_step(model, val_loader, fold, epoch, epochs, criterion, threshold, device)

            # metrics_ep_avg = sum(metrics_val[:4])/4
            if val_auc > metric_best:
        ...
        TP,FP,FN,TN,fpr,tpr, test_auc, test_aupr, test_f1_score, test_accuracy, test_recall, test_specificity, test_precision = eval_step(checkpoint, test_loader, fold, epoch, epochs, criterion, threshold, device)
        print('TP:',TP)
        print('FP:',FP)
        print('FN:',FN)
        print('TN:',TN)
